...
class DatasetClient(TaskThread):
    ...
    def initVars__(self):
        ...
        if self.cache is not None:
            ...
            if os.path.exists(self.cache.binary_file) and os.path.exists(self.cache.index_file):
                mode = 'r'
            else:
                mode = 'w'

            self.cache.record = BinaryBlob(
                binary_file=self.cache.binary_file,
                index_file=self.cache.index_file,
                mode=mode
            )

        self.logger.info("initVars__: {}".format(self.getInfo()))

    @verbose
    async def enter__(self):
        self.tasks.init_connection = await reCreate(self.tasks.init_connection, self.init_connection__)
        self.logger.debug("enter__: {}".format(self.getInfo()))
    ...
    async def init_connection__(self):
        ...
        try:
            try:
                ...
                self.logger.info("init_connection__: connect failed with", e)
                await self.reset_connection__()
                if self.cur_attempt <= self.max_retry:
                    self.logger.info(f"init_connection__: trying to reconnect within {self.retry_interval} secs")
            ...
            self.logger.info("init_connection__ : cancelling %s", self.getInfo())
        except Exception as e:
    ...
    async def exit__(self):
        ...
        self.logger.debug(f"{self.getInfo()} exit__: bye!")
    ...
    async def write_socket__(self, obj=None):
        ...
        try:
            ...
            if obj is not None:
                ...
                self.write_buf = utils.object_to_bytes(obj)
                self.write_start = 0
            ...
            self.logger.info("write_socket__ : cancelling %s", self.getInfo())
        except Exception as e:
            await self.warn_and_exit('write_socket__', str(e))
